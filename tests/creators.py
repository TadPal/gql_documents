...
def compare(
    ...
    rowb,
    attributeNames,
    skipattributes=["lastchange", "created", "createdBy", "changedBy"],
):
    for attName in attributeNames:
        if attName in skipattributes:
            continue
...
def createReferenceQuery(gqltype, attributeNames):
    attlist = " ".join(attributeNames)
    query = (
        "query($id: ID!) { _entities(representations: [{ __typename: "
        + f'"{gqltype}", id: $id'
        + " }])"
        + "{"
        + f"...on {gqltype}"
...
def createByIdTestApp(tableName, queryByIdEndpoint, attributeNames=["id", "name"]):
    ...
    async def result_test():
        ...
        for datarow in datatable:
            ...
            resp = client.post(
                "/gql", json={"query": query, "variables": variable_values}
            )
...
def createPageTest(
    tableName, queryEndpoint, attributeNames=["id", "name"], subEntities=[]
):
    # attlist = ' '.join(attributeNames)
    @pytest.mark.asyncio
    async def result_test():
        ...
        query = createPageQuery(
            queryEndpoint=queryEndpoint, attributeNames=(attributeNames + subEntities)
        )
        print("query", query)
        print("queryEndpoint", queryEndpoint)
        context_value = await createContext(async_session_maker)
...
def createPageTestApp(tableName, queryEndpoint, attributeNames=["id", "name"]):
    ...
    async def result_test():
        ...
        query = createPageQuery(
            queryEndpoint=queryEndpoint, attributeNames=attributeNames
        )
        variable_values = {}
...
def createResolveReferenceTest(
    tableName, gqltype, attributeNames=["id", "name"], subEntities=[]
):
    @pytest.mark.asyncio
    async def result_test():
        async_session_maker = await prepare_in_memory_sqllite()
        ...
        data = get_demodata()
        table = data[tableName]
        query = createReferenceQuery(
            gqltype, attributeNames=(attributeNames + subEntities)
        )
        ...
        for row in table:
            ...
            variable_values = {"id": rowid}
            resp = await schema.execute(
                query2, context_value=context_value, variable_values=variable_values
            )
            print(resp, flush=True)
            assert resp.errors is None
            data = resp.data
...
def createResolveReferenceTestApp(tableName, gqltype, attributeNames=["id", "name"]):
    ...
    async def result_test():
        ...
        for row in table:
            ...
            resp = client.post(
                "/gql", json={"query": query2, "variables": variable_values}
            )
            assert resp.status_code == 200
            resp = resp.json()
            logging.info(resp)
