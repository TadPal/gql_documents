...
class ExternalIdGQLModel:
    ...
    async def id_type(self, info: strawberry.types.Info) -> "ExternalIdTypeGQLModel":
        result = await ExternalIdTypeGQLModel.resolve_reference(
            info=info, id=self.typeid_id
        )
        return result
...
class ExternalIdInsertGQLModel:
    inner_id: strawberry.ID = strawberry.field(
        default=None, description="Primary key of entity which new outeid is assigned"
    )
    typeid_id: strawberry.ID = strawberry.field(
        default=None, description="Type of external id"
    )
    outer_id: strawberry.ID = strawberry.field(
        default=None, description="Key used by other systems"
    )
...
class ExternalIdResultGQLModel:
    ...
    async def externalid(
        self, info: strawberry.types.Info
    ) -> Union[ExternalIdGQLModel, None]:
...
async def externalid_insert(
    self, info: strawberry.types.Info, externalid: ExternalIdInsertGQLModel
) -> ExternalIdResultGQLModel:
    ...
    rows = await loader.filter_by(
        ...
        typeid_id=externalid.typeid_id,
...
async def externalid_delete(
    self, info: strawberry.types.Info, externalid: ExternalIdUpdateGQLModel
) -> ExternalIdResultGQLModel:
    loader = getLoaders(info).externalids
    result = ExternalIdResultGQLModel()
    rows = await loader.filter_by(
